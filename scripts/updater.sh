...
if [[ "${DO_PULL}" -eq 1 ]]; then
  if [[ -d "${REPO_ROOT}/.git" ]]; then
    ...
    if ! (cd "${REPO_ROOT}" && git pull --ff-only); then
      ...
      # previous image — a half-updated tree must never ship. `--no-pull`
      # (or `NO_PULL=1`) is the escape when the operator has already
      # pulled by hand and just wants the rebuild leg.
